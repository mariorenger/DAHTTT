...
export class ExamService {
  ...
  // hannn
  public searchUserList(page: number, size: number, examId: number): Observable<PageResult<UserAccount>> {
    const pageParams = new HttpParams().set('page', page.toString()).set('size', size.toString());
    return this.http.get<PageResult<UserAccount>>(`${this.baseUrl}/exams/exam-user/user-list/${examId}`, {params: pageParams});
  }
  // hannn
  public searchAllUser(page: number, size: number, examId: number): Observable<PageResult<UserAccount>> {
    const pageParams = new HttpParams().set('page', page.toString()).set('size', size.toString());
    return this.http.get<PageResult<UserAccount>>(`${this.baseUrl}/exams/exam-user/user-all/${examId}`, {params: pageParams});
  }

  // hann
  public addUserToCourse(examId: number, userId: number): Observable<any> {
    return this.http.get(`${this.baseUrl}/exams/exam-user/user-list/${examId}/${userId}`);
  }
