...
import com.httt.backend.dto.*;
import com.httt.backend.entity.*;
import com.httt.backend.service.*;
...
//    @GetMapping(value = "/exams/list-all-by-exam/{examId}")
//    public ResponseEntity<List<User>> getAllByExam(@PathVariable Long examId) throws ParseException {
//        List<User> userList = userService.findAllByExamId(examId);
...
//    hannn
    @GetMapping(value = "/exams/exam-user/user-list/{examId}")
    public PageResult getUserListById(@PathVariable Long examId, @PageableDefault(page = 0, size = 10, sort = "id") Pageable pageable) throws ParseException {
        return new PageResult(userService.findUsersByExam(examId, pageable));
    }

    //    hannn
    @GetMapping(value = "/exams/exam-user/user-all/{examId}")
    public PageResult getAllUserById(@PathVariable Long examId, @PageableDefault(page = 0, size = 10, sort = "id") Pageable pageable) throws ParseException {
        return new PageResult(userService.findAllUserByExam(examId, pageable));
    }

//    hannn
    @GetMapping(value = "/exams/exam-user/user-list/{examId}/{userId}")
    public ResponseEntity<Exam> addUserToCourse(@PathVariable Long examId, @PathVariable Long userId){
        User user = userService.findUserById(userId).get();
        System.out.println(user);
        Exam exam = examService.getExamById(examId).get();
        System.out.println(exam);
        examUserService.addUsertoExam(exam, user);
