...
import com.httt.backend.repository.PasswordResetTokenRepository;
import com.httt.backend.repository.UserRepository;
import com.httt.backend.ultilities.ERole;
import com.httt.backend.config.JwtUtils;
import com.httt.backend.dto.UserExport;
import com.httt.backend.entity.PasswordResetToken;
...
//    hannn
    @Override
    public Page<User> findUsersByExam(Long id, Pageable pageable) {
        return userRepository.findListUserByExamId(id, pageable);
    }

    //    hannn
    @Override
    public Page<User> findAllUserByExam(Long id, Pageable pageable) {
        return userRepository.findAllUserByExamId(id, pageable);
    }
...
//        Create new user
    ...
    @Override
    public User createUserHaveIntake(User user, Intake intake) {
//        Create new user
        User newUser = new User(user.getUsername(), passwordEncoder.encode(user.getUsername()), user.getEmail(), user.getProfile());
...
//    @Override
//    public List<User> findAllByExamId(Long id) {
//        return userRepository.findAllByExamId(id);
//    }
...
//        Update user password into database
        User user2 = passwordResetToken.getUser();
